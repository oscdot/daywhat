// lucia.ts
import { Adapter, KeySchema, LuciaErrorConstructor, SessionSchema, UserSchema, lucia } from 'lucia'
...
const convexAdapter = (db: DatabaseWriter) => {
  return (LuciaError: LuciaErrorConstructor): Adapter => ({
    ...
    async updateSession(sessionId: string, partialSession: Partial<SessionSchema>): Promise<void> {
      const session = await getSession(db, sessionId)
      if (session === null) {
